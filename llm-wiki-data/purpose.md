...
LLM Wiki v0.6.11 renders KaTeX only when inline formulas use `$...$` and display formulas use `$$...$$`. Use those delimiters in chat replies and archived Markdown. Do not use `\(...\)` or `\[...\]`. Inside a math delimiter, keep LaTeX subscripts and superscripts unescaped, such as `N_q` and `X_T^\top`; do not rewrite them as `N\_q` or `X\_T`.
